...
{% if mdb.use_ppa or mdb.use_repo %}
  ...
  pkgrepo.managed:
    ...
    # http://repo.mongodb.org/apt/{{ os }} {{ code }}/mongodb-org/stable {{ mdb.repo_component }}
    - name: deb http://repo.mongodb.org/apt/{{ os }} {{ code }}/{{ mdb.mongodb_package }}/{{ mdb.release }} {{ mdb.repo_component }}
...
mongodb_db_path:
  file.directory:
    - name: {{ mdb.db_path }}
    # changed username and group to vagrant as the resource is on the host
    - user: vagrant
    - group: vagrant
    - mode: 755
    - makedirs: True
